...
import {
  ...
  X, Bell, LogOut, Star, Archive, Trash2,
...
interface PatientInfo {
  ...
  fullName: string;
  email: string;
  profile: { personal: { profilePhoto: string } };
}
...
const mapApiMessageToMessage = (apiMsg: ApiMessage, myUserId: string): Message => ({
  ...
  senderRole: apiMsg.sender.userId === myUserId ? "DOCTOR" : "USER",
...
const uploadFile = async (file: File): Promise<string> => {
  ...
  if (response?.errros) throw new Error("Server error! Please try again.")
...
const ConversationItem = ({
  ...
  const name = conv.patient?.fullName ?? "Loading…";
  const initials = getInitials(name);
  const photoUrl = conv.patient?.profile?.personal?.profilePhoto;
...
const DoctorChatPage = () => {
  ...
  const filtered = conversations.filter(
    (c) =>
      (c.patient?.fullName ?? "").toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const fetchPatientInfo = async (patientId: string): Promise<PatientInfo | null> => {
    try {
      const response = await userServiceApi.findById({ input: { userId: patientId } }, `
        id fullName email profile { personal { profilePhoto } }
      `);
      return response?.data?.findUserById ?? null;
    } catch (err) {
  ...
  const fetchConversations = useCallback(async () => {
    try {
      ...
      const convs: Conversation[] = apiConvs.map((c) => {
        const patientParticipant = c.participants.find((p) => p.role === "USER");
        return {
          id: c.id,
          patientId: patientParticipant?.userId ?? "",
          patient: null,
      ...
      convs.forEach(async (conv) => {
        if (!conv.patientId) return;
        const patient = await fetchPatientInfo(conv.patientId);
        if (patient) setConversations((prev) => prev.map((c) => (c.id === conv.id ? { ...c, patient } : c)));
      });
      return convs;
    } catch (err) {
  ...
  const fetchMessages = useCallback(async (conversationId: string) => {
    try {
      const user = await doctorServiceApi.findOne({ fields: `id` });
      const uid: string | undefined = user?.data?.findDoctor?.id;
      if (!uid) return;
  ...
  useEffect(() => {
    ...
    fetchConversations().then((convs) => {
      if (patientId) {
        const existing = convs.find((c) => c.patientId === patientId);
        if (existing) {
          setGhost(null);
          ...
          fetchPatientInfo(patientId).then((patient) => {
            if (patient) setGhost({ patientId, patient });
  ...
  useEffect(() => {
    ...
    const cleanup = onReceiveMessage((data: any) => {
      ...
      setConversations((prev) =>
        prev.map((c) => {
          const isMatch = (data.conversationId && c.id === data.conversationId) || c.patientId === data.senderId;
          if (!isMatch) return c;
          return {
            ...c,
  ...
  const handleSelectConversation = (convId: string) => {
    ...
    if (!conv) return;
    if (conv.patientId) router.push(`?id=${conv.patientId}`, { scroll: false });
    setConversations((prev) => prev.map((c) => (c.id === convId ? { ...c, unread: 0 } : c)));
    if (!conv.messagesLoaded) fetchMessages(convId);
  };

  const handleTyping = useCallback(() => {
    const receiverId = active?.patientId ?? ghost?.patientId;
  ...
  const sendMessage = useCallback(async () => {
    ...
    const targetPatientId = active?.patientId ?? ghost?.patientId;
    ...
    if (!active && ghost) {
      ...
      const pollForConversation = async (retries = 10, delay = 600) => {
        for (let i = 0; i < retries; i++) {
          ...
          const created = convs.find((c) => c.patientId === targetPatientId);
          if (created) {
            setGhost(null);
            setActiveConvId(created.id);
            fetchMessages(created.id);
            router.push(`?id=${targetPatientId}`, { scroll: false });
            return;
          }
        }
        ...
        {!active && ghost ? (
          <>
            <header className="flex items-center justify-between px-4 py-3 border-b border-border bg-background">
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-3">
                  <Avatar initials={getInitials(ghost.patient.fullName)} photoUrl={ghost.patient?.profile?.personal?.profilePhoto} />
                  <div>
                    <p className="font-semibold text-sm text-foreground">{ghost.patient.fullName}</p>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center px-6">
              <Avatar initials={getInitials(ghost.patient.fullName)} photoUrl={ghost.patient.profile?.personal?.profilePhoto} size="lg" />
              <h3 className="mt-3 font-semibold text-foreground text-base">{ghost.patient.fullName}</h3>
              <p className="text-sm text-muted-foreground mt-4 max-w-xs">
                This is the beginning of your conversation with {ghost.patient.fullName}. Send a message to get started.
              </p>
            </div>
            ...
            <InputBar {...inputBarProps} placeholder={`Message ${ghost.patient.fullName}…`} />
          </>
        ) : active ? (
          <>
            <header className="flex items-center justify-between px-4 py-3 border-b border-border bg-background">
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => setShowInfo((s) => !s)} className="flex items-center gap-3 hover:opacity-80 transition-opacity">
                  <Avatar initials={getInitials(active.patient?.fullName ?? "??")} photoUrl={active.patient?.profile?.personal?.profilePhoto} />
                  <div>
                    <p className="font-semibold text-sm text-foreground">{active.patient?.fullName ?? "Loading…"}</p>
                    <p className="text-xs text-muted-foreground flex items-center gap-1">
                      {isTyping ? <span className="text-primary animate-pulse">typing…</span> : "Patient"}
                    </p>
                  </div>
                </button>
              </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center px-6">
            ...
            <h2 className="font-semibold text-foreground text-lg mb-1">
              {ghostLoading ? "Loading patient info…" : "Your Messages"}
            </h2>
            <p className="text-sm text-muted-foreground max-w-xs">
              {ghostLoading ? "Please wait a moment." : conversationsLoading ? "Loading your conversations…" : "Select a conversation from the list to start messaging a patient."}
            </p>
          </div>
        )}
      </main>
      ...
      {showInfo && active && (
        <aside className="w-72 border-l border-border bg-background flex flex-col overflow-y-auto">
          ...
          <div className="flex flex-col items-center py-6 px-4 border-b border-border">
            <Avatar initials={getInitials(active.patient?.fullName ?? "??")} photoUrl={active.patient?.profile?.personal?.profilePhoto} size="lg" />
            <h3 className="mt-3 font-semibold text-foreground text-base">{active.patient?.fullName ?? "Patient"}</h3>
          ...
          <div className="p-4 space-y-3">
            ...
            {[
              { icon: Trash2, label: "Delete chat", destructive: true },
