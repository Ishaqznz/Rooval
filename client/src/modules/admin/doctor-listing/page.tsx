...
import { DoctorListingProps } from '@/interfaces/admin.interface';
...
const DoctorListing: React.FC<DoctorListingProps> = ({ doctors = sampleDoctors }) => {
  ...
  // Approve doctor
  const approveDoctor = (doctorId: string): void => {
  ...
  // Reject doctor
  const rejectDoctor = (doctorId: string): void => {
  ...
  const allowReapply = (doctorId: string): void => {
    setDoctorList(prevDoctors =>
      prevDoctors.map(doctor =>
        doctor.id === doctorId
          ? { ...doctor, status: 'pending', applicationDate: new Date().toISOString().split('T')[0] }
          : doctor
      )
    );
    setDropdownOpen(null);
  ...
  // Delete doctor
  const deleteDoctor = (doctorId: string): void => {
  ...
  const statusConfig = {
    ...
    const config =
      ...
      { bg: 'bg-gray-200', text: 'text-gray-700', icon: AlertCircle }; // fallback
