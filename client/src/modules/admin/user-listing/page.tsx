...
import { User } from '@/interfaces/common/admin.interface';
import { UserListingProps } from '@/interfaces/common/admin.interface';
...
const UserListing: React.FC<UserListingProps> = ({ users = sampleUsers }) => {
  ...
  const toggleUserStatus = async (userId: string): Promise<void> => {
    ...
    console.log('user id when blocking the user: ', userId);
    const user = userList.find(u => u.id === userId);
    console.log('user afte finding the user Id: ', userId)
    if (!user) return;

    const prevStatus = user.status;

    if (prevStatus == 'active') {
      if (!window.confirm('Are sure want to block the user!')) return;
    }

    if (prevStatus == 'blocked') {
      if (!window.confirm('Are u sure to unblock the user!')) return;
    }

    const newStatus = prevStatus === "active" ? "blocked" : "active";
    const status = newStatus === "blocked" ? true : false;

    console.log('status of the user. It should be boolean value: ', status)

    const CHANGE_STATUS_QUERY = {
    ...
    try {
      const response = await apiRequest(CHANGE_STATUS_QUERY, 'change_status_request!');
  ...
  const deleteUser = async (userId: string): Promise<void> => {
    ...
    const QUERY = {
      query: DELETE_USER.query,
      variables: {
        input: { userId }
      }
    }

    const response = await apiRequest(QUERY, 'delete_user');
    console.log('data from the back end in the delete user: ', response.deleteUser)
    setDropdownOpen(null);
  };
                        ...
                        <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg z-20 border border-gray-200">
                          <div className="py-1">
                            ...
                            <button
                              ...
                              className={`flex items-center px-4 py-2 text-sm w-full text-left ${user?.status === 'active'
