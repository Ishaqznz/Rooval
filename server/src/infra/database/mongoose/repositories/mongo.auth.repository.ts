...
export class MongoAuthRepository implements IAuthRepository {
  ...
  async forgotPassword(email: string): Promise<boolean> {
    ...
    const doctorData = await this._doctorModel.findOne({ email })
  ...
  async resetPassword(password: string, userId: string): Promise<User | Doctor> {
    ...
    const updatedDoctor = await this._doctorModel
      .findByIdAndUpdate(userId, { password }, { new: true })
  ...
  async googleLogin(data: GoogleLogin): Promise<User | Doctor> {
    const doctor = await this._doctorModel.findOne({ email: data.email })
    if (doctor) {
    ...
    const Model = data.role === 'user' ? this._userModel : this._doctorModel;
